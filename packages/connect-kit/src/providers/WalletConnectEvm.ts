...
async function initWalletConnectProvider(): Promise<WalletConnectProvider> {
  ...
  if (
    providerOptions.optionalMethods &&
    Array.isArray(providerOptions.optionalMethods)
  )
    optionalMethods.push(
      ...providerOptions.optionalMethods.filter(
        (id) => optionalMethods.indexOf(id) < 0
      )
    );
  ...
  if (
    ...
    optionalEvents.push(
      ...providerOptions.optionalEvents.filter(
        (id) => optionalEvents.indexOf(id) < 0
      )
    );
